...
function nuclearcraftoptimize_relaxed(base_energy, base_heat,reactor_width, reactor_length, reactor_height, reactor_cell_limit = 20000;
    ...
    @variable(model,0<=upward_boosted_reactor[1:reactor_width,1:reactor_length,1:reactor_height]<=1)
    @variable(model,0<=upward_neutron_flux[1:reactor_width,1:reactor_length,1:reactor_height]<=1) #Actually can be unbounded below but for simplicity, let's bound the values.
    ...
    #Downward boosted.
    @variable(model,0<=downward_boosted_reactor[1:reactor_width,1:reactor_length,1:reactor_height]<=1)
    @variable(model,0<=downward_neutron_flux[1:reactor_width,1:reactor_length,1:reactor_height]<=1)
    ...
    #Forward.
    @variable(model,0<=forward_boosted_reactor[1:reactor_width,1:reactor_length,1:reactor_height]<=1)
    @variable(model,0<=forward_neutron_flux[1:reactor_width,1:reactor_length,1:reactor_height]<=1)
    ...
    #Backward
    @variable(model,0<=backward_boosted_reactor[1:reactor_width,1:reactor_length,1:reactor_height]<=1)
    @variable(model,0<=backward_neutron_flux[1:reactor_width,1:reactor_length,1:reactor_height]<=1)
    ...
    #Leftward.
    @variable(model,0<=leftward_boosted_reactor[1:reactor_width,1:reactor_length,1:reactor_height]<=1)
    @variable(model,0<=leftward_neutron_flux[1:reactor_width,1:reactor_length,1:reactor_height]<=1)
    ...
    #Rightward.
    @variable(model,0<=rightward_boosted_reactor[1:reactor_width,1:reactor_length,1:reactor_height]<=1)
    @variable(model,0<=rightward_neutron_flux[1:reactor_width,1:reactor_length,1:reactor_height]<=1)
    ...
    @variable(model,0<=water_coolants[1:reactor_width,1:reactor_length,1:reactor_height]<=1)
    @constraint(model,sum(water_coolants) <= water_limit)
    @variable(model,0<=redstone_coolants[1:reactor_width,1:reactor_length,1:reactor_height]<=1)
    @constraint(model,sum(redstone_coolants) <= redstone_limit)
    @variable(model,0<=quartz_coolants[1:reactor_width,1:reactor_length,1:reactor_height]<=1)
    @constraint(model,sum(quartz_coolants) <= quartz_limit)
    @variable(model,0<=gold_coolants[1:reactor_width,1:reactor_length,1:reactor_height]<=1)
    @constraint(model,sum(gold_coolants) <= gold_limit)
    @variable(model,0<=glowstone_coolants[1:reactor_width,1:reactor_length,1:reactor_height]<=1)
    ...
    return (value.(reactor_cells), value.(moderators), value.(water_coolants),
    value.(redstone_coolants),
    value.(quartz_coolants),
    value.(gold_coolants),
    value.(glowstone_coolants),
    value.(lapis_coolants),
    value.(diamond_coolants),
    value.(liquid_helium_coolants),
    value.(enderium_coolants),
    value.(cryotheum_coolants),
    value.(iron_coolants),
    value.(emerald_coolants),
    value.(copper_coolants),
    value.(tin_coolants),
    value.(magnesium_coolants))
end
...
function nuclearcraftoptimize(base_energy, base_heat,reactor_width, reactor_length, reactor_height, reactor_cell_limit = 20000;
    ...
    relaxation_time_limit = time_limit*0.8
    time_limit -= relaxation_time_limit
    reactor_value, moderator_value, water_coolant_value,redstone_coolant_value,quartz_coolant_value,gold_coolant_value,glowstone_coolant_value,lapis_coolant_value,diamond_coolant_value,liquid_helium_coolant_value,enderium_coolant_value,cryotheum_coolant_value,iron_coolant_value,emerald_coolant_value,copper_coolant_value,tin_coolant_value,magnesium_coolant_value = 
    nuclearcraftoptimize_relaxed(base_energy,base_heat,reactor_width,reactor_length,reactor_height,reactor_cell_limit,num_threads=num_threads, time_limit = relaxation_time_limit, water_cooling=water_cooling, water_limit=water_limit,redstone_cooling=redstone_cooling, redstone_limit=redstone_limit,quartz_cooling=quartz_cooling, quartz_limit=quartz_limit,gold_cooling=gold_cooling, gold_limit=gold_limit,glowstone_cooling=glowstone_cooling, glowstone_limit=glowstone_limit,lapis_cooling=lapis_cooling, lapis_limit=lapis_limit,diamond_cooling=diamond_cooling, diamond_limit=diamond_limit,liquid_helium_cooling=liquid_helium_cooling, liquid_helium_limit=liquid_helium_limit,enderium_cooling=enderium_cooling, enderium_limit=enderium_limit,cryotheum_cooling=cryotheum_cooling, cryotheum_limit=cryotheum_limit,iron_cooling=iron_cooling, iron_limit=iron_limit,emerald_cooling=emerald_cooling, emerald_limit=emerald_limit,copper_cooling=copper_cooling, copper_limit=copper_limit,tin_cooling=tin_cooling, tin_limit=tin_limit,magnesium_cooling=magnesium_cooling, magnesium_limit=magnesium_limit)
    for x in 1:reactor_width, y in 1:reactor_length, z in 1:reactor_height
        if reactor_value[x,y,z] ≈ 1.0
            @constraint(model,reactor_cells[x,y,z] == 1.0)
        end
        if reactor_value[x,y,z] ≈ 0.0
            @constraint(model,reactor_cells[x,y,z] == 0.0)
        end
        if moderator_value[x,y,z] ≈ 1.0
            @constraint(model,moderators[x,y,z] == 1.0)
        end
        if moderator_value[x,y,z] ≈ 0.0
            @constraint(model,moderators[x,y,z] == 0.0)
        end
        if water_coolant_value[x,y,z] ≈ 1.0
            @constraint(model,water_coolants[x,y,z] == 1.0)
        end
        if water_coolant_value[x,y,z] ≈ 0.0
            @constraint(model,water_coolants[x,y,z] == 0.0)
        end
        if redstone_coolant_value[x,y,z] ≈ 1.0
            @constraint(model,redstone_coolants[x,y,z] == 1.0)
        end
        if redstone_coolant_value[x,y,z] ≈ 0.0
            @constraint(model,redstone_coolants[x,y,z] == 0.0)
        end
        if quartz_coolant_value[x,y,z] ≈ 1.0
            @constraint(model,quartz_coolants[x,y,z] == 1.0)
        end
        if quartz_coolant_value[x,y,z] ≈ 0.0
            @constraint(model,quartz_coolants[x,y,z] == 0.0)
        end
        if gold_coolant_value[x,y,z] ≈ 1.0
            @constraint(model,gold_coolants[x,y,z] == 1.0)
        end
        if gold_coolant_value[x,y,z] ≈ 0.0
            @constraint(model,gold_coolants[x,y,z] == 0.0)
        end
        if glowstone_coolant_value[x,y,z] ≈ 1.0
            @constraint(model,glowstone_coolants[x,y,z] == 1.0)
        end
        if glowstone_coolant_value[x,y,z] ≈ 0.0
            @constraint(model,glowstone_coolants[x,y,z] == 0.0)
        end
        if lapis_coolant_value[x,y,z] ≈ 1.0
            @constraint(model,lapis_coolants[x,y,z] == 1.0)
        end
        if lapis_coolant_value[x,y,z] ≈ 0.0
            @constraint(model,lapis_coolants[x,y,z] == 0.0)
        end
        if diamond_coolant_value[x,y,z] ≈ 1.0
            @constraint(model,diamond_coolants[x,y,z] == 1.0)
        end
        if diamond_coolant_value[x,y,z] ≈ 0.0
            @constraint(model,diamond_coolants[x,y,z] == 0.0)
        end
        if liquid_helium_coolant_value[x,y,z] ≈ 1.0
            @constraint(model,liquid_helium_coolants[x,y,z] == 1.0)
        end
        if liquid_helium_coolant_value[x,y,z] ≈ 0.0
            @constraint(model,liquid_helium_coolants[x,y,z] == 0.0)
        end
        if enderium_coolant_value[x,y,z] ≈ 1.0
            @constraint(model,enderium_coolants[x,y,z] == 1.0)
        end
        if enderium_coolant_value[x,y,z] ≈ 0.0
            @constraint(model,enderium_coolants[x,y,z] == 0.0)
        end
        if cryotheum_coolant_value[x,y,z] ≈ 1.0
            @constraint(model,cryotheum_coolants[x,y,z] == 1.0)
        end
        if cryotheum_coolant_value[x,y,z] ≈ 0.0
            @constraint(model,cryotheum_coolants[x,y,z] == 0.0)
        end
        if iron_coolant_value[x,y,z] ≈ 1.0
            @constraint(model,iron_coolants[x,y,z] == 1.0)
        end
        if iron_coolant_value[x,y,z] ≈ 0.0
            @constraint(model,iron_coolants[x,y,z] == 0.0)
        end
        if emerald_coolant_value[x,y,z] ≈ 1.0
            @constraint(model,emerald_coolants[x,y,z] == 1.0)
        end
        if emerald_coolant_value[x,y,z] ≈ 0.0
            @constraint(model,emerald_coolants[x,y,z] == 0.0)
        end
        if copper_coolant_value[x,y,z] ≈ 1.0
            @constraint(model,copper_coolants[x,y,z] == 1.0)
        end
        if copper_coolant_value[x,y,z] ≈ 0.0
            @constraint(model,copper_coolants[x,y,z] == 0.0)
        end
        if tin_coolant_value[x,y,z] ≈ 1.0
            @constraint(model,tin_coolants[x,y,z] == 1.0)
        end
        if tin_coolant_value[x,y,z] ≈ 0.0
            @constraint(model,tin_coolants[x,y,z] == 0.0)
        end
        if magnesium_coolant_value[x,y,z] ≈ 1.0
            @constraint(model,magnesium_coolants[x,y,z] == 1.0)
        end
        if magnesium_coolant_value[x,y,z] ≈ 0.0
            @constraint(model,magnesium_coolants[x,y,z] == 0.0)
        end
